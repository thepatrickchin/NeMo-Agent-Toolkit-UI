...
export const processIntermediateMessage = (
  ...
  const replaceMessage = (steps: IntermediateStep[]): boolean => {
    for (let i = 0; i < steps.length; i++) {
      if (
        steps[i].id === newMessage.id &&
        steps[i].content?.name === newMessage.content?.name
      ) {
...
export const generateContentIntermediate = (
    ...
    try {
      ...
      return data
        .map((item) => {
          ...
          const sanitizedPayload = convertBackticksToPreCode(
            item.content?.payload || '',
          );
          let details = `<details id=${currentId} index=${currentIndex}>\n`;
          details += `  <summary id=${currentId}>${
            item.content?.name || ''
          }</summary>\n`;
  ...
  try {
    ...
    if (firstStep && firstStep.parent_id) {
      intermediateContent = `<details id=${uuidv4()} index="-1" ><summary id=${
        firstStep.parent_id
